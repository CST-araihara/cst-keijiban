...
// スレッドを復元（delete_flag = 0 にする）updated_date = CURRENT_TIMESTAMP
function thread_recover_flag($thread_id){
    $dbh = connect();
    $stmt = $dbh->prepare("UPDATE thread 
                            SET thread.delete_flag = 0
                            WHERE thread.id = '".$thread_id."';
                            ");
    $stmt->execute();
}
...
// レスを復元（delete_flag = 0 にする）
function response_recover_flag($response_id){
    $dbh = connect();
    $stmt = $dbh->prepare("UPDATE response 
                            SET response.delete_flag = 0
                            WHERE response.id = '".$response_id."';
                            ");
    $stmt->execute();
}

//必要なページ数を求める admin_threadtab
